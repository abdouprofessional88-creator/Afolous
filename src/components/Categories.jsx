...
export default function Categories() {
  const { activeCategory, setActiveCategory, setSearch } = useStore();
  const pick = (id) => {
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 -mt-2 pt-10">
      ...
      <Reveal delay="reveal-d1" className="mt-6 flex gap-3 overflow-x-auto no-scrollbar pb-2 md:grid md:grid-cols-8 md:overflow-visible">
        {CATEGORIES.map((c) => {
          ...
          return (
            <button
              ...
              className={`shrink-0 min-w-[104px] md:min-w-0 rounded-3xl border-2 px-3 py-4 flex flex-col items-center gap-2 transition active:scale-95 ${
                active ? 'bg-ink-950 text-white border-ink-950 shadow-card' : 'bg-white text-ink-900 border-ink-900/10 hover:border-ink-950 shadow-soft'
              }`}
            >
              <span className={`w-11 h-11 rounded-2xl flex items-center justify-center ${active ? 'bg-brand-500 text-white' : 'bg-cream-100 text-ink-900'}`}>
                <Icon size={22} />
              </span>
              <span className="text-[13px] font-black leading-4">{c.nameAr}</span>
              <span className={`text-[10px] font-bold uppercase tracking-wider ${active ? 'text-white/60' : 'text-ink-900/40'}`}>{c.nameEn}</span>
            </button>
          );
        })}
      </Reveal>
